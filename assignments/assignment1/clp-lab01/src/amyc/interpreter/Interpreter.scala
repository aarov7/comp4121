...
object Interpreter extends Pipeline[(Program, SymbolTable), Unit] {
  ...
  def run(ctx: Context)(v: (Program, SymbolTable)): Unit = {
    ...
    def interpret(expr: Expr)(implicit locals: Map[Identifier, Value]): Value = {
      expr match {
        case Variable(name) =>
          locals(name) //this could throw an exception if name does not exist in locals, do we need to implement that?
        case IntLiteral(i) =>
          IntValue(i) //not sure why this is not IntValue(interpret(i).asInt)
        ...
        case Times(lhs, rhs) =>
          IntValue(interpret(lhs).asInt * interpret(rhs).asInt)
        case Div(lhs, rhs) =>
          if(interpret(rhs).asInt == 0) ctx.reporter.fatal("")
        ...
        case LessEquals(lhs, rhs) =>
          BooleanValue(interpret(lhs).asInt <= interpret(rhs).asInt)
        case And(lhs, rhs) =>
          BooleanValue(interpret(lhs).asBoolean && interpret(rhs).asBoolean) //using && here and not &, because the specification mentions that it has to be short-circuiting
        case Or(lhs, rhs) =>
          BooleanValue(interpret(lhs).asBoolean || interpret(rhs).asBoolean) // using || instead of |
        case Equals(lhs, rhs) =>
          interpret(rhs) match
        ...
        case Call(qname, args) =>
          ...
          if(isConstructor(qname)) CaseClassValue(qname, args.map(argument => interpret(argument)))
            ...
            val paramList = findFunction(owner, qname.name).params.map(elem => elem.name)
            val arguments = args.map(elem => interpret(elem))
            val finalArgs = paramList.zip(arguments)
            interpret(findFunction(owner, qname.name).body)(finalArgs.toMap)
          }
            ...
            ((v, pat): @unchecked) match {
              ...
              case (CaseClassValue(con1, realArgs), CaseClassPattern(con2, formalArgs)) =>
                if(con1 != con2) None
                  ...
                  val argumentPairs = realArgs.zip(formalArgs);
                  val returnedVals = argumentPairs.map(argPair =>  matchesPattern(argPair._1, argPair._2))
                  if(returnedVals.contains(None)) None
                  else Some(returnedVals.flatten.flatten)
                }
            }
          }
