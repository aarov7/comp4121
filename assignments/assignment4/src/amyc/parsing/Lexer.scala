...
object Lexer extends Pipeline[List[File], Iterator[Token]]
  ...
  val lexer = Lexer(
    ...
    oneOf("+-*/%<-!")|word("<=")|word("&&")|word("&&")|word("||")|word("==")|word("++")
      |> { (cs, range) => OperatorToken(cs.mkString).setPos(range._1) },

    // TODO: Identifier
    elem(_.isLetter) ~ many(elem(_.isLetterOrDigit)|elem(_=='_')) //add _ here
    ...
    elem(_.isDigit) ~ many(elem(_.isDigit))
      ...
      (cs, range) =>
        try {
          ...
          case e: Exception => ErrorToken("Integer out of bounds").setPos(range._1)
        }
    },


    // TODO: String literals
    word("\"") ~ many(elem(c =>  c != '\n' && c != '"')) ~ opt(word("\""))  // All words within quotes excluding new lines
      |> {(cs, range) => {
      if (cs.drop(1).mkString.endsWith("\"")){
        StringLitToken(cs.drop(1).dropRight(1).mkString).setPos(range._1) // Drop the opening and closing quotations
      } else {
        ErrorToken("String not Closed Properly").setPos(range._1)
      }
    }},
    ...
    // Multiline comments
    word("/*") ~ many(elem(_!='*')|many1(elem(_=='*'))~elem(_!='/')) ~ opt(word("*/"))
      |> {(cs, range) =>{
      if (cs.mkString.endsWith("*/")){
        CommentToken(cs.mkString("")).setPos(range._1)
      } else {
        ErrorToken("Comment not Closed Properly").setPos(range._1)
      }
    }},
